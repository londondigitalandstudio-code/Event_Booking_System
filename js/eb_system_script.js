...
/* =========================
   📅 DATE AVAILABILITY + BLOCK PAST DATES
========================= */

// 🔒 Disable previous dates (allow only today and future)
...
// Optional: Prevent manual past date typing
eventDate.addEventListener("change", () => {
  const selectedDate = eventDate.value;
  const today = new Date().toISOString().split("T")[0];

  if (selectedDate < today) {
    alert("You cannot select a past date. Please choose today or future date.");
    eventDate.value = "";
    eventDate.focus();
  }
});
...
async function bookOrder() {
  ...
  // 🔥 Show single-line message instantly
  const statusLine = document.getElementById("bookingStatus");
  statusLine.innerText = "Your event booking...";
  statusLine.style.display = "block";

  // Allow browser to paint immediately
  ...
  if (alreadyBooked) {
    statusLine.style.display = "none";   // ✅ IMPORTANT FIX
  ...
  try {
    ...
    statusLine.style.display = "none";
    statusLine.innerText = "Booking failed. Please try again.";
